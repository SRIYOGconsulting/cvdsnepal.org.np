    ...
    <div className="min-h-screen bg-gray-50 py-6 px-2 md:px-10 relative">
          ...
          {days.map((day, index) => {
            ...
            return (
              <div
                ...
                className={`min-h-[70px] sm:min-h-[100px] p-1 sm:p-2 flex flex-col border border-gray-100 relative ${day === null ? "bg-gray-50" : "bg-white cursor-pointer"} ${isToday(day) ? "border-blue-500 border-2" : ""}`}
              >
                {day && (
                  <>
                    <span className="text-[10px] sm:text-xs text-gray-500 mb-1">{day}</span>
                    {dayHasEvents && <div className="absolute top-1 right-1 w-2 h-2 bg-blue-500 rounded-full"></div>}
                  </>
                )}
              </div>
            );
      ...
      {bannerEvents && (
        <div className="fixed md:absolute bottom-0 md:top-10 left-0 md:left-0 z-50 w-full md:w-80 max-h-[60vh] md:max-h-none overflow-y-auto p-4 bg-white border border-gray-200 rounded-t-lg md:rounded-md shadow-lg transition-all duration-300 ease-in-out"> 
          {/* x changed: fixed bottom on mobile, absolute on desktop */}
